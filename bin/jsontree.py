...
def main():
    try:       
        ...
        parser.add_argument("--node-name", help="Lambda expression to generate node names from a node, defaults to `lambda node: node.name`",
        ...
        parser.add_argument("--size", help="Size of the tree", action="store_true")
        parser.add_argument("--siblings", help="Show siblings", type=str)
        parser.add_argument("--children", help="Show children", type=str, nargs=1)
        parser.add_argument("--parent", help="Show parent", type=str)
        parser.add_argument("--ancestors", help="Show ancestors", type=str)
        parser.add_argument("--descendants", help="Show descendants", type=str, nargs=1)
        parser.add_argument("--has-node", help="Check if node exists", nargs=1, type=str)
        parser.add_argument("--get-node", help="Get node by key", type=str, nargs="?")
        parser.add_argument("--has-edge", help="Check if edge exists", type=str, nargs=2)
        parser.add_argument("--is-connected", help="Check if two nodes are connected", type=str, nargs=2)
        parser.add_argument("--get-path", help="Find the path between two nodes", type=str, nargs=2)
        ...
        if args.output:
            ...
            print(tree.to_string(node_name=node_name, fallback_node_name=fallback_node_name))
