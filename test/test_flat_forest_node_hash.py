...
class TestFlatNodeHash(unittest.TestCase):
    ...
    def test_name_hash(self):
        ...
        self.assertEqual(
            NodeHasher.name(self.node_a), NodeHasher.name(FlatForestNode(name="a"))
        )
        self.assertNotEqual(
            NodeHasher.name(self.node_a),
            NodeHasher.name(FlatForestNode(name="b", data1=1, data2=2)),
        )

    def test_payload_hash(self):
        # Test that the payload hash of two nodes with different payloads is not the same
        self.assertNotEqual(
            NodeHasher.payload(self.node_a), NodeHasher.payload(self.node_b)
        )

        # Test that the payload hash of two nodes with the same payload is the same
        self.assertNotEqual(
            NodeHasher.payload(self.node_a), NodeHasher.payload(TreeNode(name="a"))
        )

        self.assertEqual(
            NodeHasher.payload(self.node_a),
        ...
        self.assertEqual(
            NodeHasher.payload(self.node_a), NodeHasher.payload(self.node_a)
        )
    ...
    def test_node_hash(self):
        ...
        self.assertEqual(
            NodeHasher.node(self.node_a),
            NodeHasher.node(FlatForestNode(name="a", data1=1, data2=2)),
        )
        ...
        self.assertNotEqual(
            NodeHasher.node(self.node_a),
            NodeHasher.node(FlatForestNode(name="a", data1=1, data2=2, more=None)),
        )
