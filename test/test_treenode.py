...
class TestTreeNode(unittest.TestCase):
    ...
    def test_node_method(self):
        children = [{TreeNode.NAME_KEY: "child1", "value": 1},
    ...
    def test_repr(self):
        root = TreeNode(name="root", value=10)
        self.assertEqual(repr(root), "TreeNode(name=root, payload={'value': 10})")
        child = TreeNode(name="child", value=1, parent=root)
        self.assertEqual(repr(child), "TreeNode(name=child, parent=root, payload={'value': 1})")
